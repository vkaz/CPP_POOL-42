...
void Bureaucrat::signForm(Form &rhs) const
{
    try
	{
        ...
        if (rhs.check() == 0)
			...
			str = "(Signed)";
		rhs.beSigned(*this);
		std::cout << "Bureaucrat " << this->_name << " (Grade " << this->getGrade() << ") signs a " << 
            rhs.getName() <<  " (s.grade " <<
			rhs.getMin()<< ", ex.grade " << rhs.getMax() << ") targeted on " << rhs.getName() << " " << str <<  std::endl;
	}
...
void Bureaucrat::executeForm(Form &form) const
{
	try
	{
        ...
        if (form.check() == 0)
			...
			str = "(Signed)";
		form.execute(*this);
		std::cout << "Bureaucrat " << this->_name << " (Grade " << this->getGrade() << ") executes a " << 
            form.getName() <<  " (s.grade " <<
			form.getMin()<< ", ex.grade " << form.getMax() << ") targeted on " << form.getName() << " " << str << std::endl;
	}
	catch (std::exception &e)
	{
